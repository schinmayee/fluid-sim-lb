...
def launch_experiment():
    ...
    workers = instances[0:num_instances]
    ...
    print(commands.getstatusoutput(
      "gcloud compute scp --zone %s %s ./launch_lb_application.sh %s:~/" %
      (args.zone, project, controller)))
    print(commands.getstatusoutput(
      "gcloud compute scp --zone %s %s %s %s:~/init_file" %
    ...
    params += "CY=%d " % args.cy
    params += "CZ=%d " % args.cz
    params += "COARSE_SCALE=%d " % args.coarse_scale
    params += "WINDOW=%d " % args.window
    ...
    print(params)
    subprocess.call(
      "ssh -o StrictHostKeyChecking=no -i %s -f %s '%s ./launch_lb_application.sh'" %
